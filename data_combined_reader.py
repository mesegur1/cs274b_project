...
def load_data(
    ...
    for base in tqdm(range(0, len(accel_data), window_step)):
        ...
        # accel_w[:, 1] = butter_lowpass_filter(
        #     accel_w[:, 1], FREQ_CUTOFF, SAMPLE_RATE, 3
        # )
        # accel_w[:, 2] = butter_lowpass_filter(
        #     accel_w[:, 2], FREQ_CUTOFF, SAMPLE_RATE, 3
        # )
        # accel_w[:, 3] = butter_lowpass_filter(
        #     accel_w[:, 3], FREQ_CUTOFF, SAMPLE_RATE, 3
        # )
        # Check for zeroed windows
        if (
            is_greater_than(accel_w, MOTION_EPSILON) == True
            and accel_w.shape[0] == window
        ):
            # Compute TAC and extracted features
            tac_w = stats.mode(tac_data_labels[base : base + window], keepdims=True)[0][
                0
            ]
            feat_w = np.nan_to_num(
                feature_extraction(accel_w, prev_accel_w, sample_rate)
            )
...
if __name__ == "__main__":
    ...
    for pid in PIDS:
        # Load data from CSVs
        load_data(pid, END_INDEX, START_OFFSET, WINDOW, WINDOW_STEP, SAMPLE_RATE)
